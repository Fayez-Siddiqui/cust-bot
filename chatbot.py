...
key_1=key()
token=key_1.get_key("GROQ_API_KEY")
...
def chatbot_answer (transcript : str,query :str ) :
    ...
    class VectorStore(BaseModel):
        (
            "A vectorstore contains information about customer service"
            ", seo, social media management  etc."
        )
    ...
    router_prompt_template = (
        ...
        'Note that if a query is not SEO or customer service related, you must output "not customer service related", don\'t try to use any tool.\n\n'
        "query: {query}"
    )
# add llm here chatopenai
    llm = ChatGroq(model="llama-3.1-8b-instant", temperature=0,api_key=token)
    ...
    (
        "You are a well renowned Customer Service Bot your name is 'Cust-Bot'.\n"
        "Do not respond to queries that are not related to Customer Sercvice, SEO, PPC and social media.\n"
        "If a query is not related to the topic, acknowledge your limitations.\n"
    ...
    def retrieve_node(state: dict) -> dict[str, list[Document] | str]:
        ...
        """
        query = state["query"]
        documents = ret.sim_search(query)
